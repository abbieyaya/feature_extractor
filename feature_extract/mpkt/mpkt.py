...
class FiveTuple(object):
	...
	#print self.src, self.dst ,self.sport,self.dport,self.proto
...
class Connection(FiveTuple):
	...
	# ***************************************************************************
        ...
        if self.l5_proto == 'SSL':
            ...
            if packet.getContentType() == "17" : alters['SSL'] = SSLState.EXCHANGE_MESS
	    else : alters['SSL'] = SSLState.DISCONNECTED
	    
            if alters['SSL'] != None : self.state['SSL'] = alters['SSL']
	...
	if self.l5_proto != 'SSL' or ( self.l5_proto == 'SSL' and packet.getContentType() == "17" ) :
...
class Packet(FiveTuple):
    ...
    def __init__(self, buf):
        ...
        try:
            if buf['Ethernet'].fields['type'] != 0x800:
                raise PacketError("This is not a IP packet.")

            if buf['IP'].fields['proto'] != 6:
                raise PacketError("This is not a TCP packet.")
            
	    # Member varible
            super(Packet, self).__init__(
                buf['IP'].fields['src'], buf['IP'].fields['dst'],
                buf['TCP'].fields['sport'], buf['TCP'].fields['dport'], 'TCP'
            )
        except:
            raise PacketError("This is not a TCP packet.")
	...
	while True:
        ...
        self.content_type = 0
	if ( self.sport == 443 or self.dport == 443 ) and tls_type : 
	    self.content_type = buf['Raw'].fields['load'][0].encode('hex')
	

        self.len = buf['IP'].fields['len'] - buf['IP'].fields['ihl']*4 - buf['TCP'].fields['dataofs']*4
        self.flags = {                      # TCP flags
            'FIN': bool(buf['TCP'].fields['flags'] & dpkt.tcp.TH_FIN),
            'SYN': bool(buf['TCP'].fields['flags'] & dpkt.tcp.TH_SYN),
            'RST': bool(buf['TCP'].fields['flags'] & dpkt.tcp.TH_RST),
            'PSH': bool(buf['TCP'].fields['flags'] & dpkt.tcp.TH_PUSH),
            'ACK': bool(buf['TCP'].fields['flags'] & dpkt.tcp.TH_ACK),
            'URG': bool(buf['TCP'].fields['flags'] & dpkt.tcp.TH_URG),
            'ECE': bool(buf['TCP'].fields['flags'] & dpkt.tcp.TH_ECE),
            'CWR': bool(buf['TCP'].fields['flags'] & dpkt.tcp.TH_CWR)
        }
